...
pub const FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP : i32 = 0x1ea; //hi2
pub const FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_DIVE : i32 = 0x1eb;  //hi3
...
pub const DIVE_TRANSITION_MANUAL : bool = true;
pub const DIVE_TRANSITION_FRAME_MIN : f32 = 5.0;
pub const DIVE_TRANSITION_FRAME_FORCE : f32 = 45.0;
pub const DIVE_HI4_MAX_FRAME : f32 = 7.0;
...
pub unsafe extern "C" fn special_hi_common_init(fighter: &mut L2CFighterCommon, status_kind: i32) {
    ...
    else if (status_kind == FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP) {
    ...
    let max_y_param = if (status_kind == FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_DIVE) {hash40("air_speed_y_stable")} else {hash40("dive_speed_y")};
    ...
    println!("SpeedY: {speed_y} Motion Y:{motion_y}");
    if !KineticModule::is_enable_energy(fighter.module_accessor, *FIGHTER_KINETIC_ENERGY_ID_GRAVITY) {
        KineticModule::enable_energy(fighter.module_accessor, *FIGHTER_KINETIC_ENERGY_ID_GRAVITY);
        let new_speed_y = if (status_kind == FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP) {speed_y*FLIP_SPEED_Y_MUL} else {DIVE_SPEED_Y_START};
        sv_kinetic_energy!(
            reset_energy,
            fighter,
    ...
    if (status_kind == FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP) {
...
pub unsafe extern "C" fn special_hi_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let jump_speed_mul = 1.0;//WorkModule::get_param_int(fighter.module_accessor, hash40("param_special_hi"), hash40("jump_speed_mul"));
    sv_kinetic_energy!(
        set_speed_mul,
        fighter,
...
pub unsafe extern "C" fn special_hi_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_ROY_STATUS_SPECIAL_HI_FLAG_SPECIAL_HI_SET_LR) {
        ...
        println!("Set LR? {stick_x} {turn_stick}");
    ...
    if !is_jump {
        ...
        if MotionModule::is_end(fighter.module_accessor) {
            println!("Aborted during ground?");
            fighter.change_status_by_situation(FIGHTER_STATUS_KIND_LANDING_FALL_SPECIAL.into(), FIGHTER_STATUS_KIND_FALL_SPECIAL.into(), false.into());
            return 1.into();
        }
    }
    else{
        if MotionModule::is_end(fighter.module_accessor) {
            fighter.change_status(FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_DIVE.into(), false.into());
            return 1.into();
        }
        //check for flip out
        if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_ROY_STATUS_SPECIAL_HI_FLAG_FREE_FALL_CHK) {
            ...
            if !ControlModule::check_button_on(fighter.module_accessor, *CONTROL_PAD_BUTTON_SPECIAL) {
                fighter.change_status(FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP.into(), false.into());
                return 1.into();
            }
        }
    }
...
pub unsafe extern "C" fn special_hi_exec(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !is_jump {
        ...
        let motion_y: f32 = MotionModule::trans_move_speed(fighter.module_accessor).value[1];
        //println!("Energy:{move_speed_y} Motion: {motion_y}");
        ...
        if motion_y > 0.0 || WorkModule::is_flag(fighter.module_accessor, *FIGHTER_ROY_STATUS_SPECIAL_HI_FLAG_TRANS_JUMP) {
            ...
            if WorkModule::is_flag(fighter.module_accessor,*FIGHTER_ROY_STATUS_SPECIAL_HI_FLAG_TRANS_MOVE_SET_ANGLE) {
                ...
                println!("Stick {}",stick_x*lr);

                let angle = (RISE_ANGLE_BASE + (RISE_ANGLE_MUL*stick_x))*-1.0;
                println!("X: {stick_x} LR {lr} Angle: {angle}");
...
pub unsafe extern "C" fn special_hi_2_init(fighter: &mut L2CFighterCommon) -> L2CValue {
    special_hi_common_init(fighter,FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP);
    0.into()
}
...
pub unsafe extern "C" fn special_hi_3_init(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    special_hi_common_init(fighter,FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_DIVE);
    0.into()
}
...
pub unsafe extern "C" fn special_fall_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    let prev_kind =StatusModule::prev_status_kind(fighter.module_accessor, 0);
    if [FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP,FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_DIVE].contains(&prev_kind) {
        special_hi_common_init(fighter,prev_kind);
        fighter.sub_fighter_cliff_check(L2CValue::I32(*GROUND_CLIFF_CHECK_KIND_ALWAYS_BOTH_SIDES));
        println!("Fall init");
    }
    return fighter.status_fall_special()
}
...
pub fn install(agent: &mut smashline::Agent) {
    ...
    agent.status(Init, FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP, special_hi_2_init);
    agent.status(Pre, FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP, special_hi_2_pre);
    agent.status(Main, FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP, special_hi_2_main);
    agent.status(Exec, FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP, special_hi_2_exec);
    agent.status(End, FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP, special_hi_end);
    agent.status(Exit, FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_FLIP, special_hi_exit);

    agent.status(Init, FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_DIVE, special_hi_3_init);
    agent.status(Main, FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_DIVE, special_hi_3_main);
    agent.status(CheckAttack, FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_DIVE, special_hi_3_attack);
    agent.status(Exit, FIGHTER_ROY_STATUS_KIND_SPECIAL_HI_DIVE, special_hi_exit);

    //agent.status(Main, *FIGHTER_STATUS_KIND_FALL_SPECIAL, special_fall_main);
