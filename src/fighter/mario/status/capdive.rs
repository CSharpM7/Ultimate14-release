...
pub unsafe extern "C" fn capdive_main(fighter: &mut smashline::L2CFighterCommon) -> L2CValue {
    ...
    if fighter.is_situation(*SITUATION_KIND_GROUND) {
        ...
        let dive_max_speed_x = 1.5*lr;
        let dive_speed_y = 0.125;//0.75;
        sv_kinetic_energy!(
            reset_energy,
            fighter,
...
pub fn install(agent: &mut smashline::Agent) {
    agent.status(Pre, mario::STATUS_KIND_CAPDIVE, capdive_pre);
    agent.status(Main, mario::STATUS_KIND_CAPDIVE, capdive_main);
    agent.status(End, mario::STATUS_KIND_CAPDIVE, capdive_end);
}
