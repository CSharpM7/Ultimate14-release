...
pub unsafe extern "C" fn capjump_main(fighter: &mut smashline::L2CFighterCommon) -> L2CValue {    
    ...
    sv_kinetic_energy!(
        ...
        fighter,
        FIGHTER_KINETIC_ENERGY_ID_CONTROL,
        air_speed_x_stable * air_max_speed_x_mul,
        0.0
    );
    sv_kinetic_energy!(
        set_limit_speed,
...
pub fn install(agent: &mut smashline::Agent) {
    agent.status(Pre, mario::STATUS_KIND_CAPJUMP, capjump_pre);
    agent.status(Main, mario::STATUS_KIND_CAPJUMP, capjump_main);
    agent.status(End, mario::STATUS_KIND_CAPJUMP, capjump_end);
}
