...
pub fn install(agent: &mut smashline::Agent) {  
    ...
    hold::install(agent);
    hop::install(agent);
    swallowed::install(agent);
    turn::install(agent);
    jump::install(agent);
    pocket::install(agent);
}
...
unsafe extern "C" fn captoss_attacked_check(weapon: &mut smashline::L2CWeaponCommon) -> bool{
    ...
    let was_hit = !AttackModule::is_attack(weapon.module_accessor, 0, false);
...
unsafe extern "C" fn captoss_delete_if_orphaned(weapon: &mut smashline::L2CWeaponCommon) -> bool{
    ...
    if toreturn == true {
        ...
        weapon.clear_lua_stack();
        smash_script::notify_event_msc_cmd!(weapon, Hash40::new_raw(0x199c462b5d));
        if StatusModule::status_kind(weapon.module_accessor) != mario_cappy::CAPTOSS_STATUS_KIND_START {
...
unsafe extern "C" fn captoss_check_recapture(weapon: &mut smashline::L2CWeaponCommon) -> bool {
    ...
    && cap_team == owner_team
    {
        ...
        let turn_cap = (cap_status == mario_cappy::CAPTOSS_STATUS_KIND_TURN && speed_current.abs() < speed_max*0.375);
        
        let enabled = VarModule::is_flag(owner_object, mario::instance::flag::CAPJUMP_ENABLED);
        
        if [*FIGHTER_STATUS_KIND_CATCH_WAIT,*FIGHTER_STATUS_KIND_CATCH_ATTACK,*FIGHTER_STATUS_KIND_THROW].contains(&owner_status) 
        && cap_status == mario_cappy::CAPTOSS_STATUS_KIND_HOLD {
            return false;
        }
        ...
        || [mario::STATUS_KIND_CAPDIVE].contains(&owner_status);

        let can_transition = enabled && can_cap && can_catch && (![*FIGHTER_STATUS_KIND_JUMP,*FIGHTER_STATUS_KIND_JUMP_AERIAL,*FIGHTER_STATUS_KIND_FLY,
            mario::STATUS_KIND_CAPDIVE].contains(&owner_status));

        let jump_count = WorkModule::get_int(owner_boma, *FIGHTER_INSTANCE_WORK_ID_INT_JUMP_COUNT);
        let jump_count_max = WorkModule::get_int(owner_boma, *FIGHTER_INSTANCE_WORK_ID_INT_JUMP_COUNT_MAX);

        //println!("Damaged: {is_damaged} Enabled: {enabled} CapStatus: {cap_status} CanCap: {can_cap} Catch: {can_catch} Trans: {can_transition}");
        if enabled
        && (cap_status == mario_cappy::CAPTOSS_STATUS_KIND_HOLD || (turn_cap) || (owner_status == mario::STATUS_KIND_CAPDIVE && can_cap)) 
        //&& !is_damaged 
        ...
        && can_catch
        {
            if captoss_distance_to_owner(weapon) < min_dis - 3.0 {
                if VarModule::is_flag(owner_object, mario::instance::flag::CAPJUMP_ENABLED) {
                    //WorkModule::on_flag(weapon.battle_object, mario_cappy::CAPDIVE_ENABLE_ON_RETURN);
                }
                VarModule::off_flag(owner_object, mario::instance::flag::CAPJUMP_ENABLED);
                VarModule::off_flag(owner_object, mario::instance::flag::CAPDIVE_ENABLED);
                ...
                //StatusModule::change_status_force(owner_boma, *FIGHTER_STATUS_KIND_JUMP_AERIAL, false);
                StatusModule::change_status_force(owner_boma, mario::STATUS_KIND_CAPJUMP, false);

                let jump_count_new = (jump_count+1).clamp(1,jump_count_max);
                WorkModule::set_int(owner_boma, jump_count_new,*FIGHTER_INSTANCE_WORK_ID_INT_JUMP_COUNT);

                StatusModule::change_status_force(weapon.module_accessor, mario_cappy::CAPTOSS_STATUS_KIND_JUMP, false);
            ...
            if can_transition {
                StatusModule::change_status_force(owner_boma, mario::STATUS_KIND_CAPCATCH, false);
