...
pub fn install(agent: &mut smashline::Agent) {  
    agent.status(Pre, mario_cappy::CAPTOSS_STATUS_KIND_JUMP, captoss_jump_pre);
    agent.status(Main, mario_cappy::CAPTOSS_STATUS_KIND_JUMP, captoss_jump_main);
    agent.status(End, mario_cappy::CAPTOSS_STATUS_KIND_JUMP, captoss_jump_end);
}